...
### Что сносим
- Весь код в `backend/` и `frontend/`
- Скрипты деплоя
- cursorrules
- .cursor/

### Что сохраняем
- `docs/` — вся документация (обновляем по ходу)
- `ТЗ_JIRA_CUT.txt` — исходное ТЗ
- `.git` — история коммитов
- `.github/` — CI/CD (обновляем)

### Порядок действий
1. Создать ветку `rebuild/v2` от текущего main
2. Удалить старый код (кроме docs/)
3. Инициализировать новую структуру
4. Начать Sprint 1
...
- Chrome 139+
- Yandex Browser 25+
- Linux: Red OS, Astra Linux (целевые)
- PostgreSQL 14+ (целевой 16)
